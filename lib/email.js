  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f5;padding:32px 16px">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px">
        ...
        <!-- Header -->
        <tr><td style="background:linear-gradient(135deg,#e11d48,#f59e0b);border-radius:12px 12px 0 0;padding:28px 32px;text-align:center">
...
function btn(href, text) {
  return `<a href="${href}" style="display:inline-block;padding:13px 28px;background:linear-gradient(135deg,#e11d48,#f59e0b);color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:14px;margin:8px 0">${text} &rarr;</a>`;
}
...
export async function sendWelcomeEmail(to, name, role) {
  ...
  return send(to, `Welcome to RubRhythm, ${name}!`, layout(`
    ...
    <p>${btn(`${SITE}/${isProvider ? "myaccount/listings/add-listing" : "united-states"}`, isProvider ? "Create Your Listing" : "Browse Providers")}</p>
    ${isProvider ? `
    <div style="margin-top:20px;padding:16px 20px;background:#fef9f0;border-left:4px solid #f59e0b;border-radius:0 8px 8px 0">
      <p style="color:#92400e;font-weight:700;font-size:13px;margin:0 0 6px">Quick Start</p>
...
export async function sendReviewNotificationEmail(to, providerName, listingTitle, rating) {
  ...
  return send(to, `New ${rating}-Star Review on "${listingTitle}"`, layout(`
    ...
    <div style="text-align:center;padding:20px;background:#fafafa;border:1px solid #e5e7eb;border-radius:10px;margin:0 0 20px">
      <p style="color:#f59e0b;font-size:30px;margin:0;letter-spacing:4px">${stars}</p>
...
export async function sendVerificationApprovedEmail(to, name) {
  return send(to, "You're Verified on RubRhythm!", layout(`
...
export async function sendCreditsConfirmedEmail(to, name, credits, amountUSD) {
  return send(to, `${credits} Credits Added to Your Account`, layout(`
...
export async function sendExpirationWarningEmail(to, name, listingTitle, featureType, expiresAt) {
  ...
  return send(to, `Your ${featureType} expires on ${dateStr}`, layout(`
    <h1 style="color:#111827;font-size:22px;margin:0 0 8px">Expiring Soon ⚠️</h1>
...
export async function sendPasswordResetEmail(to, resetUrl) {
  return send(to, "Reset your RubRhythm password", layout(`
    <div style="text-align:center;margin:0 0 20px">
      <span style="font-size:52px">🔑</span>
    </div>
...
export async function sendNoAccountEmail(to) {
  return send(to, "RubRhythm: no account found for this email", layout(`
    <div style="text-align:center;margin:0 0 20px">
      <span style="font-size:52px">📬</span>
    </div>
...
export async function sendReferralBonusEmail(to, name, credits) {
  return send(to, `You earned ${credits} credits from a referral!`, layout(`
