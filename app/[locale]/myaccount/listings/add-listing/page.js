  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (age && parseInt(age) < 21) {
        setError('You must be at least 21 years old to create a listing.');
        setLoading(false);
        return;
      }

      if (!listingId && balance < LISTING_FEE) {
        setError(`Insufficient credits. You need ${LISTING_FEE} credits to post a listing.`);
        setLoading(false);
        return;
      }
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Failed to save listing');
      }
    } catch (error) {
  ...
  const processFiles = async (files) => {
    ...
    try {
      ...
      if (listingId) {
        ...
        formData.append('listingId', generatedId);
      }
      ...
      if (response.ok) {
        ...
        if (data.listingId && !listingId && !generatedId) {
          setGeneratedId(data.listingId);
        }
      ...
      <div className="bg-[#0d0d15] rounded-3xl p-6 md:p-8 border border-white/10 shadow-2xl backdrop-blur-xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 z-10 w-full">
          <div className="w-20 h-20 bg-gradient-to-tr from-primary to-accent rounded-2xl flex items-center justify-center shadow-lg border border-white/10 shrink-0">
            <svg className="w-8 h-8 text-white drop-shadow-md" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-[#0d0d15] rounded-3xl p-6 md:p-8 border border-white/10 shadow-2xl relative overflow-hidden group hover:border-white/20 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
            ...
            <div>
              <label htmlFor="al-bodytype" className={LabelStyle}>{t('bodyType')} <span className="text-white/30 text-[10px] font-normal">(optional)</span></label>
        ...
        <div className="bg-[#0d0d15] rounded-3xl p-6 md:p-8 border border-white/10 shadow-2xl relative overflow-hidden group hover:border-white/20 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
            ...
            <div className="flex gap-3">
              <div className="w-[100px] shrink-0">
                ...
                <input id="al-phonearea" type="tel" value={phoneArea} onChange={(e) => setPhoneArea(e.target.value.replace(/\D/g, ''))} className={InputStyle} required placeholder="407" maxLength="3" inputMode="numeric" />
              </div>
              <div className="flex-1">
                <label htmlFor="al-phone" className={LabelStyle}>{t('phoneNumber')} <span className="text-red-500">*</span></label>
                <input id="al-phone" type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value.replace(/\D/g, ''))} className={InputStyle} required placeholder="5551234" maxLength="7" inputMode="numeric" />
              </div>
            </div>

            <div className="flex items-end">
              <label className="flex items-center space-x-3 cursor-pointer bg-white/5 p-4 rounded-xl border border-white/10 hover:border-green-500/50 transition-all w-full">
          ...
          <details className="mt-6 group/opt">
            <summary className="flex items-center gap-2 cursor-pointer text-white/50 hover:text-white/70 transition-colors py-3 border-t border-white/10">
              <svg className="w-4 h-4 transition-transform group-open/opt:rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              <span className="text-sm font-bold uppercase tracking-wider">Optional details</span>
              <span className="text-[10px] text-white/30">(neighborhood, website, social links)</span>
            </summary>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="al-twitter" className={LabelStyle}>
