...
const SERVICE_ICONS = {
  "available-now": <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" strokeWidth={2}/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6l4 2"/></svg>,
  "bump-up": <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 11l5-5m0 0l5 5m-5-5v12"/></svg>,
  "auto-bump": <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>,
  "highlight": <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>,
  "feature": <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>,
  "feature-premium": <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>,
};
...
const SERVICE_IDS = ["available-now", "bump-up", "auto-bump", "highlight", "feature", "feature-premium"];

export default function ServicesPage() {
  const t = useTranslations('services');
  const tm = useTranslations('myaccount');
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col justify-center items-center">
        ...
        <p className="text-white/40 animate-pulse font-medium">{tm('loadingServices')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1200px] w-full mx-auto space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pb-6 border-b border-white/10">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">{tm('boostServicesTitle')}</h1>
          <p className="text-text-muted mt-1">{tm('boostServicesSubtitle')}</p>
        </div>
        <div className="flex items-center gap-4 bg-white/5 border border-white/10 px-5 py-3 rounded-2xl">
          <div>
            <p className="text-[10px] uppercase font-bold text-text-muted tracking-wider">{tm('yourBalance')}</p>
            <p className="text-2xl font-black text-white leading-none">{balance} <span className="text-sm text-text-muted font-normal">{tm('creditsUnit')}</span></p>
          </div>
      ...
      {listings.length > 0 && (
        <div className="bg-[#0d0d15] rounded-2xl p-5 border border-white/10">
          <p className="text-sm font-bold text-white mb-3">{tm('selectListingToBoost')}</p>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${
                    isSelected ? 'bg-primary/10 border-primary/40 shadow-[0_0_15px_rgba(255,42,127,0.15)]' : 'bg-white/5 border-white/10 hover:border-white/20'
                  }`}
                >
      ...
      {listings.length === 0 && (
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h3 className="text-amber-400 font-bold text-base">{tm('noActiveListings')}</h3>
            <p className="text-amber-300/70 text-sm">{tm('noActiveListingsDesc')}</p>
          </div>
          ...
          return (
            <div key={service.id} className={`bg-[#0d0d15] rounded-3xl p-6 border ${c.border} ${c.hoverBorder} transition-all group relative overflow-hidden flex flex-col`}>
                ...
                <div className="flex flex-col gap-1.5 mb-4">
                  {service.pricingDetails.map((p, i) => (
                    <div key={i} className="flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg border border-white/10 text-xs">
                      ...
                      <span className="font-black text-yellow-400">{p.cost} {tm('creditsUnit')}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {service.id === "auto-bump" ? (
                <Link href="/myaccount/listings/auto-bump" className={`w-full text-center px-5 py-2.5 border font-bold rounded-xl transition-all block text-sm ${c.btn}`}>
                  {tm('configureAutoBump')}
                </Link>
              ) : selectedListing ? (
                <Link href={`/myaccount/listings/boost?id=${selectedListing.id}`} className={`w-full text-center px-5 py-2.5 border font-bold rounded-xl transition-all block text-sm ${c.btn}`}>
                  {t('boost')} "{selectedListing.title?.substring(0, 20)}{selectedListing.title?.length > 20 ? '...' : ''}"
                </Link>
              ) : (
                <Link href="/myaccount/listings/add-listing" className="w-full text-center px-5 py-2.5 border border-white/10 bg-white/5 text-white/40 font-bold rounded-xl transition-all block text-sm hover:border-white/20 hover:text-white/60">
                  {tm('createListingFirst')}
                </Link>
              )}
            </div>
          );
      ...
      <div className="bg-[#0d0d15] rounded-3xl p-6 md:p-8 border border-white/10">
        <h3 className="text-lg font-black text-white mb-4">{tm('quickComparison')}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 text-white/40 text-xs uppercase tracking-wider">
                <th className="text-left py-3 pr-4">{tm('thService')}</th>
                <th className="text-center py-3 px-2">{tm('thCost')}</th>
                <th className="text-center py-3 px-2">{tm('thDuration')}</th>
                <th className="text-center py-3 px-2">{tm('thEffect')}</th>
              </tr>
            </thead>
